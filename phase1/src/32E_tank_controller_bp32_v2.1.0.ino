// ============================================================
// ESP32 Tank Mower — Phase 2 Firmware  v2.1.0
...
//   - Onboard WS2811 RGB LED (GPIO 16) mirrors PS4 controller LED
//     colours in real time via FastLED
//
// v2.1.0 changes:
//   - PI_SERIAL_RX moved GPIO 16 → GPIO 13 (GPIO 16 reserved for WS2811)
...
//   PCF8575 I/O expander via I2C (SDA=GPIO 21, SCL=GPIO 22)
//   WS2811 RGB LED on GPIO 16 (onboard, 1 LED)
//
...
// ── WS2811 onboard RGB LED ───────────────────────────────────
#define WS2811_PIN    16    // Onboard WS2811 data pin (GPIO 16)
#define WS2811_COUNT  1     // One LED on board

// ── I2C — PCF8575 expander ───────────────────────────────────
#define I2C_SDA       21    // Default ESP32 SDA
#define I2C_SCL       22    // Default ESP32 SCL
#define PCF8575_ADDR  0x20  // A0/A1/A2 all LOW; change if address pins differ
PCF8575 pcf(PCF8575_ADDR);

// ── Serial1 — 2nd ESP32 peer link ────────────────────────────
#define ESP2_SERIAL_TX  4
#define ESP2_SERIAL_RX  5
#define ESP2_BAUD       115200

CRGB wsLeds[WS2811_COUNT];
...
#define PI_SERIAL_RX      13        // ESP32 RX2 ← Pi GPIO14 (TX)  [moved from 16 — WS2811]
...
const Colour COL_OFF    = { 0,   0,   0   }; // Off   — disconnected
...
// ============================================================
// LED helpers — drives both PS4 bar and onboard WS2811
// ============================================================
...
void onDisconnectedGamepad(GamepadPtr gp) {
  ...
  // Turn off onboard WS2811 on disconnect
  setLED(COL_OFF);
...
void setup() {
  Serial.begin(115200);
  delay(400);
  debugBanner("ESP32-DevKitC-32E TANK MOWER — PHASE 2 v2.0.0");
  ...
  // ── WS2811 onboard LED ────────────────────────────────
  FastLED.addLeds<WS2811, WS2811_PIN, RGB>(wsLeds, WS2811_COUNT);
  FastLED.setBrightness(128);
  wsLeds[0] = CRGB::Black;
  FastLED.show();
  Serial.print(F("[INIT] WS2811 LED ready (GPIO ")); Serial.print(WS2811_PIN); Serial.println(F(")"));
  ...
  pcf.begin();
  Serial.println(F("[INIT] I2C started (PCF8575)"));
  ...
  Serial.println(F("[INIT] Serial1 (2nd ESP32) started"));
  Serial.print(F("  TX=GPIO")); Serial.print(ESP2_SERIAL_TX);
  Serial.print(F("  RX=GPIO")); Serial.print(ESP2_SERIAL_RX);
