// ============================================================
// ESP32 Tank Mower — Phase 2 Firmware  v3.0.0
// Board: ESP32-DevKitC-32E (ESP32-WROOM-32E)
// Based on v2.5.0
//
// v3.0.0 changes:
//   - FreeRTOS dual-core split:
//       Core 1 (loop task): BP32 polling, motor drive, arm state
//         machine, LED flash, rumble, all button handling
//       Core 0 (ioTask):    Pi serial RX/TX, PCF8575 read/write,
//         Pip-Boy TX
//   - pcf.read16() (up to 25 ms I2C block) no longer starves the
//     BT/motor path — I2C exclusively on Core 0 after init
//   - Pi telemetry TX removed from the motor-loop hot path
//   - Two mutex-protected structs replace raw shared globals:
//       Ctrl2IO  Core 1 → IO task: relay state, gamepad snapshot,
//                encoder snapshot, drive outputs for scene engine
//       IO2Ctrl  IO task → Core 1: Pi drive commands, PCF sensor
//                data, PCF/relay request flags, rumble trigger
//   - Encoder ISR attached on Core 1; noInterrupts() snapshot in
//     Core 1 task remains correct
//   - Serial (USB debug) exclusively Core 1
//   - Serial1 (Pip-Boy) and Serial2 (Pi bridge) exclusively IO task
//   - Wire / PCF8575 exclusively Core 0 after Wire.begin() in setup()
//   - All pin assignments, hardware config, and JSON protocol unchanged
//
...
// Core 1 → IO task: updated every loop(), read by IO task for
// telemetry and Pip-Boy scene engine.
struct Ctrl2IO {
  bool     relay_motor;
  bool     relay_turbo;
  bool     connected;      // gGamepad != nullptr
  TankMode tankMode;
  int      batt;           // controller battery %
  int      driveL;         // raw (pre-smooth) left target — for scene
  int      driveR;
  bool     piDriveActive;  // copy for AUTONOMOUS scene
  long     enc_ticks_L;
  long     enc_ticks_R;
  float    enc_dist_L;
  float    enc_dist_R;
...
// IO task → Core 1: Pi drive commands, PCF sensor readings,
// cross-core request flags.
struct IO2Ctrl {
  // Pi autonomous drive
  bool          piDriveActive;
  int           pi_leftSpeed;
  int           pi_rightSpeed;
  unsigned long lastPiCmd;   // millis() of last drive command

  // PCF8575 sensor state (last read)
  uint8_t bat1_level;
  uint8_t bat2_level;
  bool    bat1_heat;
  bool    bat2_heat;
  ...
  // IO task → Core 1 relay/motor requests (set by IO task, cleared by Core 1)
  bool req_arm_start;
  bool req_motor_stop;
  bool req_turbo_set;   // true when IO task wants to write a new turbo state
  bool req_turbo_val;   // desired turbo state when req_turbo_set is true

  // Core 1 → IO task PCF button pulses (set by Core 1, cleared by IO task)
  bool req_lights_btn;

  // IO task → Core 1 rumble trigger (set by IO task on mower error, cleared by Core 1)
  bool req_rumble;
};

static SemaphoreHandle_t g_mutex_c2i;  // guards g_c2i
static SemaphoreHandle_t g_mutex_i2c;  // guards g_i2c
...
// ============================================================
// Debug helpers — Core 1 only (Serial is exclusively Core 1)
// ============================================================
...
void debugFullStatus() {
  // Take a snapshot of IO2Ctrl for PCF / Pi state
  IO2Ctrl snap;
...
// ============================================================
// LED helpers — PS4 controller RGB LED (Core 1 only)
// ============================================================
...
// ============================================================
// Bluepad32 callbacks (called from Core 1 BT stack context)
// ============================================================
...
void onDisconnectedGamepad(GamepadPtr gp) {
  ...
  // Tell IO task to clear in-flight PCF button pulses and Pi drive
  xSemaphoreTake(g_mutex_i2c, portMAX_DELAY);
  g_i2c.piDriveActive  = false;
  g_i2c.pi_leftSpeed   = 0;
  g_i2c.pi_rightSpeed  = 0;
  g_i2c.req_lights_btn = false;  // cancel any pending button pulse
  xSemaphoreGive(g_mutex_i2c);
...
static void publishCtrl2IO(int driveL, int driveR, bool piDriveActive) {
  // Encoder snapshot — ISRs are on Core 1, noInterrupts() is correct here
  noInterrupts();
  long  tl = enc_ticks_L;
  long  tr = enc_ticks_R;
  ...
  // Non-blocking — skip this publish if IO task holds the mutex.
  // The IO task will read a slightly stale snapshot on the next telemetry tick.
...
static void ioReadPCF(uint8_t& bat1, uint8_t& bat2,
  ...
  if (mow_err && !prev_err) {
    rumble_out = true;  // signal Core 1 to rumble
  }
  prev_err = mow_err;
}
...
static void ioParsePiCommand(const char* line, bool& piDriveActive,
  ...
  if (strstr(line, "\"cmd\":\"drive\"")) {
    // Only accepted when controller is not connected — check snapshot
    Ctrl2IO snap;
  ...
  if (strstr(line, "\"cmd\":\"relay\"")) {
    ...
    if (strstr(line, "\"id\":\"motor\"")) {
      // Read current relay_motor state from Ctrl2IO to avoid acting twice
      Ctrl2IO csnap;
      xSemaphoreGive(g_mutex_i2c);
      xSemaphoreTake(g_mutex_c2i, portMAX_DELAY);
      csnap = g_c2i;
  ...
  if (strstr(line, "\"cmd\":\"pcf\"")) {
    ...
    if (strstr(line, "\"id\":\"turbo_btn\"")) {
      ...
      // turboBtnOff timer is managed locally in IO task
    } else if (strstr(line, "\"id\":\"lights_btn\"")) {
      pcf.write(PCF_LIGHTS_BTN, LOW);
      // lightsBtnOff timer managed locally; lights toggle handled below
    }
  }
}
...
static void ioTask(void*) {
  ...
  for (;;) {
    ...
    if (pcfPresent && now - lastPCFRead >= PCF_READ_MS) {
      ...
      // Push sensor state and any rumble request to IO2Ctrl
      xSemaphoreTake(g_mutex_i2c, portMAX_DELAY);
    ...
    if (pcfPresent) {
      ...
      // Check for lights button request from Core 1 (Square button)
      xSemaphoreTake(g_mutex_i2c, portMAX_DELAY);
    ...
    while (Serial2.available()) {
      ...
      if (c == '\n') {
        piCmdBuf[piCmdLen] = '\0';
        bool oldActive = piDriveActive;
        unsigned long prevCmd = lastPiCmd;
        ioParsePiCommand(piCmdBuf, piDriveActive, pi_leftSpeed,
                         pi_rightSpeed, lastPiCmd);
        piCmdLen = 0;

        // Push updated drive state to IO2Ctrl after every command
        xSemaphoreTake(g_mutex_i2c, portMAX_DELAY);
    ...
    vTaskDelay(1);  // yield to other Core 0 tasks — 1 tick ≈ 1 ms
  }
}
...
void setup() {
  Serial.begin(115200);
  delay(400);
  debugBanner("ESP32-DevKitC-32E TANK MOWER — PHASE 2 v3.0.0");
  ...
  // ── I2C + PCF8575 — init here, then handed to Core 0 ─
  ...
  // ── AS5600 encoders — ISRs attached to Core 1 here ───
  ...
  Serial.println(F("[INIT] IO task started on Core 0"));
...
void loop() {
  ...
  // Persists across iterations — if the mutex is unavailable this
  // iteration we drive on the previous snapshot rather than blocking.
  static IO2Ctrl io = {};
  ...
  if (piDriveActive && (millis() - io.lastPiCmd > PI_CMD_TIMEOUT_MS)) {
    piDriveActive = false;
    // IO task will clear its own state via its own timeout check;
    // we just stop driving here to be safe.
  ...
  // ── 7. Autonomous drive (no controller) ──────────────
  ...
  if (curCROSS && !prev_CROSS) {
    ...
    Colour c;
    if      (b >= 75) c = {   0, 255,   0 };
    else if (b >= 50) c = { 255, 180,   0 };
    else if (b >= 25) c = { 255,  80,   0 };
    else              c = { 255,   0,   0 };
    setLED(c);
